...
class ModelsTest {
    ...
    fun `Student serializes and deserializes correctly`() {
        val student = Student(
            id = "st-1", userId = 1L, name = "Isabella García", age = 10,
    ...
    fun `Task priority enum round-trips`() {
        val task = Task("t-1", 1L, "Preparar recital", Priority.ALTA, false)
    ...
    fun `ClassEntry serializes with paid flag`() {
        val entry = ClassEntry("c-1", "st-1", "2026-05-02", "Escalas mayores", true)
    ...
    fun `Event type enum round-trips`() {
        val event = Event("e-1", 1L, "Recital de fin de año", "2026-12-15", EventType.RECITAL, "Auditorio principal")
